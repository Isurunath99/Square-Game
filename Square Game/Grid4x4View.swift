...
struct Grid4x4View: View {
    @State private var colors: [Color] = [.red, .red, .yellow, .yellow, .green, .green, .blue, .blue, .black, .black, .purple, .purple, .orange, .orange, .pink, .pink]
    @State private var shuffledColors: [Color] = []
    @State private var displayedColors: [Color] = Array(repeating: .gray, count: 16)
    ...
    func startNewGame() {
        ...
        Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { timer in
            if countdown > 0 {
                ...
                timer.invalidate()
                displayedColors = Array(repeating: .gray, count: 16)
            }
        }
    }
